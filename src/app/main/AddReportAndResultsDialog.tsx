...
export function AddReportAndResultsDialog({ sample, onClose }: AddReportAndResultsDialogProps) {
  ...
  const dialogTitle = activeStep === 0 ? 'Add a report' : 'Add residues from lab reports';
  const dialogSubtitle = activeStep === 1
    ? 'Enter every residue listed in your lab reports in the table below, then save them to ResiYou.'
    : undefined;
  ...
  return (
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth={dialogMaxWidth}
      ...
      <DialogTitle sx={{ m: 0, px: 3, pt: 3, pb: 0 }}>
        <Stack direction="row" alignItems="flex-start" justifyContent="space-between">
          ...
          <IconButton onClick={onClose} sx={{ color: 'text.secondary', p: 0.5, mt: -0.5 }}>
            <CloseIcon />
          </IconButton>
        </Stack>
      </DialogTitle>
          ...
          <DialogContent sx={{ px: 3, pt: 0, pb: 1 }}>
            <Stack direction="row" justifyContent={hasAnalyteStatus ? 'space-between' : 'flex-end'} alignItems="flex-start" sx={{ mt: 2.5, mb: 2.5 }}>
              <AnalyteImportStatus
                ...
                onRedo={() => handleImportAnalytes(missingAnalytes)}
              />
    ...
    <ConfirmActionDialog
      ...
      title="Have you added all report results?"
      body={`We found ${missingAnalytes.length} analyte${missingAnalytes.length !== 1 ? 's' : ''} in this plot’s treatments that ${missingAnalytes.length !== 1 ? 'aren’t' : 'isn’t'} in your report results yet: ${formatAnalyteList(missingAnalytes)}. Add them to ResiYou or save without them.`}
      onClose={() => setConfirmMissingOpen(false)}
      secondaryLabel="Save without them"
      onSecondary={handleSaveReport}
      primaryLabel="Add missing analytes"

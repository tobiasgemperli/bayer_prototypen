...
export function SampleFormDialog({
  ...
  const [activeStep, setActiveStep] = useState(0); // 0=Sample details, 1=Laboratory
  ...
  const canCreate = canContinueStep0 && !labMissing;
    ...
    >
      <DialogTitle sx={{ m: 0, px: 3, pt: 3, pb: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Typography variant="h6" component="div" sx={{ fontWeight: 700, fontSize: '1.25rem', lineHeight: 1.2 }}>
          {activeStep === 0 ? title : 'Choose laboratory'}
        </Typography>
          ...
          <DialogContent sx={{ px: 3, pt: 1, pb: 3 }}>
            <Stack spacing={2.5}>
              <Typography variant="body2" color="text.secondary">
                Add the sample's details below.
              </Typography>
      ...
      {activeStep === 1 && (
        <>
          <DialogContent sx={{ px: 3, pt: 1, pb: 3 }}>
            <Stack spacing={2.5}>
              <Typography variant="body2" color="text.secondary">
                Let us know where you're sending this sample so we can tailor the next steps to your needs.
              </Typography>
              <Box>
                ...
                {!labMissing && LABS_WITH_API_CONNECTION.has(values.laboratory) && (
                  <Typography sx={{ mt: 1, color: 'text.secondary', fontSize: '0.75rem' }}>
                    This lab is connected to ResiYou. Results will import automatically once submitted.
                  </Typography>
                )}
              </Box>
            </Stack>
          ...
          <DialogActions sx={{ px: 3, pb: 3, pt: 1, justifyContent: 'flex-end', gap: 1 }}>
            ...
            <Button onClick={handleCreate} variant="contained" color="primary"
              sx={{ textTransform: 'none', fontWeight: 600, px: 2, height: 36, borderRadius: '8px' }}>
              {title.toLowerCase().includes('edit') ? 'Save changes' : 'Create sample'}
            </Button>
          </DialogActions>
        </>
      )}

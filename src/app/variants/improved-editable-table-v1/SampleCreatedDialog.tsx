    ...
    >
      ...
      <DialogContent sx={{ px: 4, pt: 4, pb: 4, textAlign: 'center' }}>
        ...
        <Stack spacing={3} sx={{ maxWidth: 440, mx: 'auto', mb: 3.5 }}>
          <StepItem
            number={1}
            title="Send the sample sheet"
            description={<>Print the sample sheet and send it together with your physical sample to {labLabel}.</>}
          />
          {labHasApiConnection ? (
            <StepItem
              number={2}
              title="Get results automatically"
              description={<>As soon as {labLabel} uploads the results, ResiYou adds them here automatically and sends you an email.</>}
            />
          ) : (
            <StepItem
              number={2}
              title="Add the report once it is back"
              description={<>As soon as you receive the report from {labLabel}, come back here to add it and its results to this sample.</>}
            />
          )}
        </Stack>

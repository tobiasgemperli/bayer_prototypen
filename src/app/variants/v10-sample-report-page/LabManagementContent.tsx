...
export function LabManagementContent({ plotId }: { plotId: string }) {
  ...
  const filteredSamples = useMemo(() => {
    ...
    return samples.filter((s) =>
      ...
      (s.laboratories ?? (s.laboratory ? [s.laboratory] : [])).join(' ').toLowerCase().includes(q)
    );
  }, [samples, search]);

  const persistSample = (values: SampleFormValues, isDraft: boolean): LabSampleData => {
    const patch = { ...values, laboratory: values.laboratories[0] ?? '', isDraft };
    if (editingId) {
      updateLabSample(editingId, patch);
      return { ...(samples.find((s) => s.id === editingId) as LabSampleData), ...patch };
    }
    const fresh = createLabSample(plotId);
    updateLabSample(fresh.id, patch);
    return { ...fresh, ...patch };
  };
  ...
  const handleCreate = (values: SampleFormValues) => {
    ...
    if (!wasEditing) {
      ...
      setCreatedLabs(values.laboratories);
      setCreatedOpen(true);
    } else {
...
const handleDuplicate = (s: LabSampleData) => {
  ...
  const handleAddReportAndResults = (s: LabSampleData) => {
    navigate(`/plot/${plotId}/samples/${s.id}`);
  };
  ...
  if (samples.length === 0) {
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        ...
        <SampleCreatedDialog
          open={createdOpen}
          labHasApiConnection={createdLabs.length > 0 && createdLabs.every((l) => LABS_WITH_API_CONNECTION.has(l))}
          labName={createdLabs.length === 1 ? createdLabs[0] : undefined}
          ...
          onAddReportAndResults={() => {
            ...
            if (createdSampleId) navigate(`/plot/${plotId}/samples/${createdSampleId}`);
          }}
        />
      </Box>
    );

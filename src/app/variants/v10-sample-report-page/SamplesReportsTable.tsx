            ...
            {sorted.length === 0 ? (
              ...
              return (
                ...
                >
                  ...
                  <TableCell sx={{ fontSize: '0.875rem' }}>
                    <LabsCell labs={s.laboratories ?? (s.laboratory ? [s.laboratory] : [])} />
                  </TableCell>
...
/** Laboratory column cell — same trigger contract as LabMultiSelect's closed
 *  state: nothing selected → dash, one lab → name (+ connection chip), more
 *  than one → "N selected". */
function LabsCell({ labs }: { labs: string[] }) {
  if (labs.length === 0) return <EmDash />;
  if (labs.length === 1) {
    return (
      <NameWithChip
        name={labs[0]}
        chip={LABS_WITH_API_CONNECTION.has(labs[0]) && <ApiConnectionChip />}
      />
    );
  }
  return <Typography component="span" sx={{ fontSize: '0.875rem', color: 'text.primary' }}>{labs.length} selected</Typography>;
}

...
import {
  Box, Breadcrumbs, Button, Divider, IconButton, Link, Paper, Stack, TextField,
...
import {
  ...
  LABS_WITH_API_CONNECTION, COMMODITY_OPTIONS,
...
export function SampleReportPage() {
  ...
  const [laboratories, setLaboratories] = useState<string[]>(
    sample?.laboratories ?? (sample?.laboratory ? [sample.laboratory] : [])
  );
  ...
  const isApiLab = laboratories.length > 0 && laboratories.every((l) => LABS_WITH_API_CONNECTION.has(l));
  const labNames = laboratories.join(', ');
  ...
  const handleSave = () => {
    ...
    updateLabSample(sample.id, {
      sampleName, dateOfSample, commodity,
      laboratory: laboratories[0] ?? '', laboratories,
      comments, reports, residues, isDraft: false,
    });
    toast.success('Saved');
    handleBack();
  };
          ...
          <Paper elevation={0} sx={cardSx}>
            ...
            <Stack spacing={2}>
              ...
              <Box>
                ...
                <LabMultiSelect value={laboratories} onChange={setLaboratories} />
              </Box>
              ...
              {isApiLab ? (
                <Stack spacing={1} alignItems="center">
                  ...
                  <Typography sx={{ color: 'text.disabled', fontSize: '0.875rem', mt: 1 }}>
                    Results will be imported automatically from {labNames} once the lab submits them.
                  </Typography>
                </Stack>
              ) : (

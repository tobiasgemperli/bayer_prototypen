...
import { SamplesGrid, SamplesGridHandle } from './SamplesGrid';
import { ReportsGrid, ReportsGridHandle, ReportRow } from './ReportsGrid';
import { ResultsGrid, ResultsGridHandle, ResidueRow, reportLabel } from './ResultsGrid';
...
export function LabManagementContent({ plotId }: { plotId: string }) {
  ...
  // Force-redirect to Lab reports when the Results tab is hidden but selected
  // (e.g. user was on Results, then unticked the last attachment).
  useEffect(() => {
    if (!hasCompleteReport && subTab === 2) setSubTab(1);
  }, [hasCompleteReport, subTab]);
      ...
      <Box sx={{ px: 2, py: 1.5, borderBottom: '1px solid', borderColor: 'divider' }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <SecondaryTabs value={subTab} onChange={(_, v) => switchTab(v)}>
            ...
            {hasCompleteReport && <Tab label="Report results" />}
          </SecondaryTabs>
